...
async function attemptLogin() {
  ...
  try {
    ...
    if (users.length === 0) {
      try {
        const localUsersSetting = await db.settings.get('users');
        if (localUsersSetting && localUsersSetting.users) {
          users = typeof localUsersSetting.users === 'string' ? JSON.parse(localUsersSetting.users) : localUsersSetting.users;
        }
      } catch (e) {}
    }
    if (users.length === 0) {
      const defaultUser = { key: 'users', users: JSON.stringify([{ username: 'admin', fullName: 'Admin', pin: '1234', role: 'Owner', createdAt: Date.now() }]) };
      await db.settings.put(defaultUser);
      users = [{ username: 'admin', fullName: 'Admin', pin: '1234', role: 'Owner' }];
    }
